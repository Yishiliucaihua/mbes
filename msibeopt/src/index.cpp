...
void index_opt::insert(const unordered_map<VIT, ST> &_l, const unordered_map<VIT, ST> &_r)
{
	this->temp.clear();
	for (const auto &v : _l)
	{
...
void index_opt::insert(const vertex_index_list &_l, const vertex_index_list &_r)
{
	this->temp.clear();
	for (const auto v : _l)
	{
